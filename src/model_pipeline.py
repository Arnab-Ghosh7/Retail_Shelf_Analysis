...
class ShelfAnalysisPipeline:
    def __init__(self, yolo_model_path="models/best.pt", use_gpu=False):
        ...
        # Remove final fully connected layer to get 2048-dim embedding
    ...
    def _extract_embedding(self, crop_img):
        ...
        with torch.no_grad():
            ...
            # Shape: [1, 2048, 1, 1] -> squeeze to [2048]
    ...
    def group_brands(self, crops, eps=0.35, min_samples=2):
        ...
        # Format labels into unique brand_id strings
        brand_ids = []
        noise_counter = 0
        
        for idx, label in enumerate(labels):
            if label == -1:
                # Noise points represent unique brands or objects that don't match any cluster
    ...
    def process_image(self, image_path_or_pil, output_dir="outputs", eps=0.35, min_samples=2):
        ...
        # 2. Extract crops for detected boxes
        crops = []
        valid_indices = []
        ...
        # Filter boxes and confs to valid ones
        ...
        # Generate colors for visualization based on group ID
        ...
        # Choose font (fallback to default if custom font not available)
        try:
